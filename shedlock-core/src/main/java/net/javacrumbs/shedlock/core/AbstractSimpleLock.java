...
public abstract class AbstractSimpleLock implements SimpleLock {
    ...
    public @NonNull Optional<SimpleLock> extend(@NonNull Duration lockAtMostFor, @NonNull Duration lockAtLeastFor) {
        checkValidity();
        Optional<SimpleLock> result = doExtend(new LockConfiguration(ClockProvider.now(), lockConfiguration.getName(), lockAtMostFor, lockAtLeastFor, lockConfiguration.getVerbose()));
        valid = false;
        return result;
    }
