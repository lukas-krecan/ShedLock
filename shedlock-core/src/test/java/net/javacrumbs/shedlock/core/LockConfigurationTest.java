...
class LockConfigurationTest {
    ...
    void lockAtLeastUntilShouldBeBeforeOrEqualsToLockAtMostUntil() {
        ...
        assertThatThrownBy(() -> new LockConfiguration(now(), "name", ZERO, ofMillis(1)))
                .isInstanceOf(IllegalArgumentException.class);
    }

    @Test
    void lockAtMostUntilHasToBeInTheFuture() {
        assertThatThrownBy(() -> new LockConfiguration(now(), "name", ofSeconds(-1), ZERO))
                .isInstanceOf(IllegalArgumentException.class);
    }

    @Test
    void nameShouldNotBeEmpty() {
        assertThatThrownBy(() -> new LockConfiguration(now(), "", ofSeconds(5), ofSeconds(5)))
                .isInstanceOf(IllegalArgumentException.class);
    }
}
