...
@Testcontainers
abstract class AbstractMongoLockProviderIntegrationTest extends AbstractExtensibleLockProviderIntegrationTest {

    static final String DB_NAME = "db";

    @Container
    public static final MongoDBContainer container = new MongoDBContainer("mongo:5.0.28");
    ...
    protected void assertUnlocked(String lockName) {
        ...
        assertThat(getDate(lockDocument, LOCK_UNTIL)).isBeforeOrEqualTo(now());
        assertThat(getDate(lockDocument, LOCKED_AT)).isBeforeOrEqualTo(now());
        assertThat((String) lockDocument.get(LOCKED_BY)).isNotEmpty();
    }

    protected abstract Date getDate(Document lockDocument, String fieldName);
    ...
    protected void assertLocked(String lockName) {
        Document lockDocument = getLockDocument(lockName);
        assertThat(getDate(lockDocument, LOCK_UNTIL)).isAfter(now());
        assertThat(getDate(lockDocument, LOCKED_AT)).isBeforeOrEqualTo(now());
        assertThat((String) lockDocument.get(LOCKED_BY)).isNotEmpty();
    }
