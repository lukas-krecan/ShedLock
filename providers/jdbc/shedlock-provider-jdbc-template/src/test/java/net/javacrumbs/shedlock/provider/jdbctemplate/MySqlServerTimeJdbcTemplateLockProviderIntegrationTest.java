...
public class MySqlServerTimeJdbcTemplateLockProviderIntegrationTest extends AbstractMySqlJdbcLockProviderIntegrationTest implements ServerTimeTest {
    ...
    protected void assertUnlocked(String lockName) {
        assertThat(getTestUtils().getJdbcTemplate().queryForObject("SELECT count(*) FROM shedlock WHERE name = ? and lock_until <= UTC_TIMESTAMP(3)", new Object[]{lockName}, Integer.class)).isEqualTo(1);
    }
    ...
    public void shouldCreateLockIfRecordAlreadyExists() {
        getTestUtils().getJdbcTemplate().update("INSERT INTO shedlock(name, lock_until, locked_at, locked_by) VALUES(?, UTC_TIMESTAMP(3), UTC_TIMESTAMP(3), ?)", LOCK_NAME1, "me");
        shouldCreateLock();
    }
}
